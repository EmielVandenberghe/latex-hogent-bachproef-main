...
> **Opmerking syntax:** Grafana Unified Alerting behandelt een waarde van `0` als "geen data aanwezig" in sommige contexten, waardoor `== 0` queries niet firen. Alle alerts gebruiken daarom `(1 - X) > 0` zodat een waarde van 0 wel als alarmerend wordt herkend.
...
**Alternatief voor productie:** Webhook naar Teams/Slack/PagerDuty:
```yaml
receivers:
  - uid: teams-webhook
    type: teams
...
Na 2 minuten firet de alert "Site onbereikbaar (ICMP)".
...
Alle provisioned alert rules in de PoC gebruiken `noDataState: OK`. Dat betekent: als Grafana tijdens een evaluatie-interval geen enkel sample ontvangt voor de query, wordt de alert NIET in Firing of Alerting gezet, maar in OK. Dit is een bewuste keuze.
...
2. **Multi-laag veiligheidsnet dekt het risico af.** Het risico van `noDataState: OK` — dat een echte exporter-down niet opgemerkt wordt — is afgedekt door een aparte alert die specifiek op scrape-gezondheid kijkt:
   ...
   Als `incontrol2-exporter` crasht of geen data meer levert, firet deze regel binnen 2 minuten. De andere 8 regels hoeven dus niet individueel de "afwezigheid van data" te detecteren.

3. **Gelaagdheid per faaldomein.** Elke regel monitort één specifiek faaldomein: ICMP-regels monitoren netwerk-bereikbaarheid, SNMP-regels monitoren device-interne toestand, SRT-regels monitoren stream-kwaliteit. De `Exporter Down`-regel neemt de scrape-laag voor zijn rekening; de andere regels nemen hun eigen signal-laag.

4. **False-positive kost > false-negative kost voor deze PoC.** In een live AV-productie zijn valse alarmen duurder dan een 2-minuten vertraagde detectie bij scrape-failure, omdat de exporter-down regel dat vertragingsvenster hard begrenst. De time-to-detect-berekening in `srt_monitoring.md` (~2 minuten) houdt al rekening met deze architectuur.

### Wanneer deze keuze heroverwogen moet worden

- Als in productie een specifieke metric kritisch is voor safety (bv. een hardware-temperatuur die bij absentie alarm moet geven), kan die specifieke regel op `noDataState: Alerting` gezet worden, zonder de rest aan te passen.
