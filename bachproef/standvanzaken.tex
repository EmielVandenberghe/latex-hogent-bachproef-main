...
\begin{itemize}
\item \textbf{Round-Trip Time (RTT)}: De tijd in milliseconden die een pakket nodig heeft om van zender naar ontvanger en terug te reizen. Deze waarde is cruciaal voor het berekenen van de optimale latency-instelling. De aanbevolen SRT-latency is minimaal drie tot vier keer de gemiddelde RTT \autocite{Sharabayko2023}.
...
\item \textbf{Available buffer}: De beschikbare ruimte in de ontvangstbuffer, uitgedrukt in bytes of pakketten. Wanneer deze waarde te laag wordt, kunnen nieuwe pakketten niet worden gebufferd en gaan verloren.
\end{itemize}
...
De keuze voor de PLG-stack is gebaseerd op de volgende overwegingen:

\begin{itemize}
\item \textbf{Flexibiliteit}: Prometheus ondersteunt custom exporters, waardoor data uit de InControl2 API en SRT-statistieken eenvoudig kan worden geïntegreerd.
\item \textbf{Kosteloos}: Alle componenten zijn open-source en kunnen zonder licentiekosten worden ingezet.
\item \textbf{Actieve community}: Uitgebreide documentatie en community support verlagen de drempel voor implementatie.
\item \textbf{Schaalbaarheid}: De architectuur is geschikt voor zowel kleinschalige proof-of-concepts als grotere productieomgevingen.
\item \textbf{Industrie-standaard}: De PLG-stack wordt breed toegepast in vergelijkbare observability-projecten, wat overdraagbaarheid en herkenbaarheid ten goede komt.
\end{itemize}
